...
public class BoardManager : MonoBehaviour
{
    ...
    private void addTiles(Vector2 tileToAdd)
    {
        if (!gridPositions.ContainsKey(tileToAdd))
        {
            ...
            {
                Debug.Log(GameManager.instance.enemySpawnRatio + "enemie ratio");
                toInstantiate = enemy;
                instance = Instantiate(toInstantiate, new Vector3(tileToAdd.x, tileToAdd.y, 0f), Quaternion.identity) as GameObject;
                instance.transform.SetParent(boardHolder);
            }
            ...
            //    GameObject npcPrefab = npcManager.SpawnRandomNPC(); 
            //    if (npcPrefab != null)
            //    {
            //        instance = Instantiate(npcPrefab, new Vector3(tileToAdd.x, tileToAdd.y, 0f), Quaternion.identity) as GameObject;
            //        instance.transform.SetParent(boardHolder);
            //    }
            //}
